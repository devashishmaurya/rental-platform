...
interface AddListingBody {
  ...
  epcRating: string
  description: string
  /** When true, backend generates description; description from UI may be null. */
...
export default function AddListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.autoGenerateDescription && !form.description?.trim()) {
      setError('Please add a property description or choose Generate Smart Description.')
      return
    }
    ...
    try {
      const payload = {
        ...form,
        autoGenerateDescription: !!form.autoGenerateDescription,
        description: form.autoGenerateDescription ? null : (form.description ?? ''),
      }
      await api.post('/api/listing/add', payload, { baseURL: '' })
      setSuccess(true)
    } catch (err) {
            ...
            <div className="p-6 space-y-5">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className={labelClass.replace(' mb-2', '')}>Advert Description</span>
                  <span className="text-gray-400" title="Choose how to add your listing description">
                ...
                <div className="flex flex-wrap gap-3 mb-4">
                  <button
                    type="button"
                    onClick={() => update('autoGenerateDescription', false)}
                    className={`rounded-xl px-5 py-2.5 font-semibold text-sm transition-all ${
                      !form.autoGenerateDescription
                  ...
                  <button
                    type="button"
                    onClick={() => update('autoGenerateDescription', true)}
                    className={`rounded-xl px-5 py-2.5 font-semibold text-sm transition-all ${
                      form.autoGenerateDescription
                        ? 'bg-primary-600 text-white shadow-sm'
                        : 'bg-white border-2 border-primary-500 text-primary-600 hover:bg-primary-50'
                ...
                {!form.autoGenerateDescription && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Your description *</label>
                    <textarea
                      required={!form.autoGenerateDescription}
                      rows={5}
                ...
                {form.autoGenerateDescription && (
                  <p className="text-sm text-gray-500">
                    We&apos;ll generate a description for your listing when you submit.
                  </p>
                )}
              </div>
